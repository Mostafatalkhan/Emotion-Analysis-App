    ...
    if (response.statusCode == 200) {
      ...
      if (alternatives != null && alternatives.isNotEmpty) {
        ...
        setState(() {
          ...
          // Open the emotion detection screen with the transcription result
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => EmotionDetectorScreen(transcription: _responseText),
            ),
          );
        });
      } else {
...
class EmotionDetectorScreen extends StatefulWidget {
  ...
  const EmotionDetectorScreen({required this.transcription});
...
class _EmotionDetectorScreenState extends State<EmotionDetectorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: defaultAppBar(context, 'Emotion Detector'),
      body: Padding(
        padding: const EdgeInsets.all(20),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
